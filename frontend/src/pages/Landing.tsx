                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {filtered.map((club) => (
                    <Link key={club._id} to={`/clubs/${club._id}`}>
                      <Card
                        hover
                        className="h-full border-2 border-blue-200 hover:border-brand-400 hover:shadow-lg hover:shadow-brand-100/50 transition-all bg-white"
                      >
                        <CardContent className="p-6">
                          ...
                          <h3 className="font-bold text-blue-950 font-mono text-lg">
                            {club.name}
                          </h3>
                          <p className="mt-3 text-sm text-blue-800 line-clamp-2 leading-relaxed">
                            {club.description}
                          </p>
                          {(club.tags ?? []).length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-1">
                              {club.tags!.slice(0, 4).map((tag) => (
                                <Badge key={tag} variant="default" className="bg-blue-300! text-white">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
